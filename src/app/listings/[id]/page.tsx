...
export default async function ListingDetailPage({ params }: PageProps<"/listings/[id]">) {
  ...
  const details: { label: string; value: string }[] = [
    { label: t("propertyType"), value: tOptions(`propertyType.${listing.propertyType}`) },
    { label: t("purpose"), value: listing.purpose === "SALE" ? t("forSale") : t("forRent") },
    { label: t("area"), value: formatArea(listing.area) },
  ];
  if (listing.bedrooms !== null) details.push({ label: t("bedrooms"), value: String(listing.bedrooms) });
  if (listing.bathrooms !== null) details.push({ label: t("bathrooms"), value: String(listing.bathrooms) });
  if (listing.floor !== null) details.push({ label: t("floor"), value: String(listing.floor) });
  if (listing.compound) details.push({ label: t("compound"), value: listing.compound });
  details.push({ label: t("city"), value: cityLabel(listing.city, locale) });
    ...
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
      {listing.status !== "APPROVED" && (isOwner || isAdmin) && (
        <div className="mb-6 rounded-xl bg-amber-50 px-4 py-3 text-sm text-amber-800">
          {t("pendingNotice", {
            status: tStatus(listing.status.toLowerCase()),
            asAdmin: isAdmin && !isOwner ? t("asAdmin") : "",
          })}
        </div>
      )}
      ...
      <nav className="mb-4 text-sm text-muted-foreground">
        ...
        / <span>{cityLabel(listing.city, locale)}</span>
      </nav>
      ...
      <div className="grid grid-cols-1 gap-8 lg:grid-cols-3">
        <div className="lg:col-span-2">
            ...
            <div className="flex items-center gap-3">
              <p className="text-2xl font-bold text-primary">
                ...
                {listing.purpose === "RENT" && (
                  <span className="text-sm font-normal text-muted-foreground"> {tCard("perMonth")}</span>
                )}
              </p>
              <FavoriteButton
          ...
          {amenities.length > 0 && (
            <div className="mt-6">
              <h2 className="text-lg font-bold text-foreground">{t("amenities")}</h2>
        ...
        <div className="lg:col-span-1">
          <div className="sticky top-24 flex flex-col gap-4">
            <div className="rounded-2xl border border-border bg-surface p-5">
              <p className="text-xs text-muted-foreground">{t("listedBy")}</p>
              <p className="mt-1 font-semibold text-foreground">{listing.user.name}</p>
              <p className="text-xs text-muted-foreground">
                {listing.user.role === "AGENT" ? t("realEstateAgent") : t("propertyOwner")}
              </p>
              {listing.user.phone && (
                <a
                ...
                >
                  {t("call", { phone: listing.user.phone })}
                </a>
              )}
            </div>

            <div className="rounded-2xl border border-border bg-surface p-5">
              <h2 className="font-semibold text-foreground">{t("sendMessage")}</h2>
              <p className="mt-1 text-xs text-muted-foreground">{t("getInTouch")}</p>
              <div className="mt-4">
      ...
      {similar.length > 0 && (
        <div className="mt-14">
          <h2 className="text-lg font-bold text-foreground">{t("similarListings")}</h2>
